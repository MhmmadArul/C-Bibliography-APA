...
button.addEventListener('click', function() {
    ...
    const jdulArtikel = document.querySelector('#judulArtikel').value;

    const nmaJurnal = document.querySelector('#namaJurnal').value;

    const volume = document.querySelector('#volume').value;

    const nomor = document.querySelector('#nomor').value;

    const halaman = document.querySelector('#halaman').value;

    const show = document.querySelector('#show');
    show.innerHTML = nakh1  + ',' + (nadep1.charAt(0)) + '.' + (nateng1.charAt(0)) + '.,' + nakh2 + ',' + (nadep2.charAt(0)) + '.' + (nateng2.charAt(0)) + '.,' + nakh3 + ',' + (nadep3.charAt(0)) + '.' + (nateng3.charAt(0)) + '.' + '(' + thunTerbit + ').' + jdulArtikel + '.' + nmaJurnal  + '.' + volume + '(' + nomor + ').' + halaman + '.';
})